...
# Get x-axis limits to position annotations just outside the plot
x_max = patients_df['patient'].max()
x_min = patients_df['patient'].min()
x_range = x_max - x_min
# Position annotations just outside the plot border
annotation_x = x_max + x_range * 0.08  # 8% of range beyond max

# Add text annotations with shape symbols (positioned outside plot border)
# Aligned with their respective mean lines
ax.text(annotation_x, mean_dreamy, 
        f'Mean: ○ {mean_dreamy:.2f}', 
        color=color_dreamy, fontsize=11, fontweight='bold',
        ha='left', va='center', zorder=4)
ax.text(annotation_x, mean_duck, 
        f'Mean: △ {mean_duck:.2f}', 
        color=color_duck, fontsize=11, fontweight='bold',
        ha='left', va='center', zorder=4)
...
ax.set_xlim(x_min - x_range * 0.02, annotation_x + x_range * 0.08)
...
Since we cannot rerun history with randomization, we run an experiment to collect the data we need. So, we randomly assigned a sequence of 100 patients to either Dr. Dreamy or Dr. Duck.
...
x_min = patients_randomized_df['patient'].min()
x_range = x_max - x_min
# Position annotations just outside the plot border
annotation_x = x_max + x_range * 0.08  # 8% of range beyond max
...
x_range = x_max - x_min
# Position annotations just outside the plot border
annotation_x = x_max + x_range * 0.15  # 15% of range beyond max
...
# Adjust x-axis limits to accommodate annotations
ax.set_xlim(x_min - x_range * 0.02, annotation_x + x_range * 0.15)
...
Looking at Plot for Outcomes by Patient Severity (Stratification), the value of stratifying by severity is visually most obvious with initial patient severity scores between -1 and 1 (highlighted in the overlay). We chose this region because it shows substantial overlap between the two surgeons—both surgeons have considerable data within this range, making it an ideal region for comparing their performance. Outside of this range, there is less data available for making direct comparisons: Doc Duck sees more of the higher-severity patients (severity > 1), while Doc Dreamy sees more of the lower-severity patients (severity < -1).
